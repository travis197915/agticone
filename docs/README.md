...
The repo at a glance: a Django REST service plus two LangGraph packages.
...
* **Every LLM call** routes through `_llm_call()` in [a07_enrich.py](../uhc-sop-ingestion/src/uhc_sop_ingestion/agents/a07_enrich.py): retry × 2 → cross-provider fallback → schema validate → `LLMCallLog` row.
