...
def _merge_args(template: dict, claim: dict) -> dict:
    ...
    for key in ("subscriber_id", "member_id", "claim_id", "diagnosis_code",
                "cpt_code", "place_of_service", "first_name", "last_name", "dob"):
        if key in claim and key not in args:
            args[key] = claim[key]
    return args


def _mk_result(rule: dict, order_index: int, *,
               verdict: dict | None, meta: dict | None,
               matched: bool, skipped: bool, skip_reason: str = "") -> dict:
    ...
    evidence_refs = [str(e) for e in evidence_refs] if isinstance(evidence_refs, list) else []
    conditions = verdict.get("conditions")
    conditions = conditions if isinstance(conditions, list) else []
    navigation = verdict.get("navigation") if isinstance(verdict.get("navigation"), dict) else None
    return {
        ...
        "applicable": (False if skipped and skip_reason.startswith("not-applicable")
...
def execute_shapes(state: ExecutionState) -> dict:
    ...
    def _evaluate(rule: dict) -> dict:
        ...
        res = _mk_result(
            rule, order_box[0], verdict=verdict, meta=meta,
            matched=matched, skipped=skipped,
            skip_reason=("not-applicable: applicable_when not satisfied" if skipped else ""),
        )
    ...
    def _mark_skipped(rules: list[dict], reason: str, step_no: Any) -> None:
        ...
        for rule in rules:
            rule_results.append(_mk_result(
                rule, order_box[0], verdict=None, meta=None,
                matched=False, skipped=True, skip_reason=reason,
            ))
            order_box[0] += 1
        if rules:
            publish_event("step_skipped", {
                "claim_id": claim_id,
                "step_number": step_no,
    ...
    for shape in shapes:
        for rule in (shape.get("rules") or []):
            (preconditions if rule.get("source") == "precondition" else decisions).append(rule)
    ...
    for rule in preconditions:
        ...
        if verdict["_matched"] and rule.get("decision_type") in _HALT_DECISION_TYPES:
            ...
            stages.append({"node": "execute_shapes", "status": "OK",
                           "ms": int((time.time() - t0) * 1000),
                           "msg": f"precondition halt at {rule['key']}"})
            return _finish(rule_results, stages, terminated_at_shape_id,
                           tool_invocations, tool_results)

    # ── build the step index from decisions ───────────────────────────────────
    ...
    summary_step = ordered_steps[-1] if ordered_steps else None
    ...
    while i < len(ordered_steps) and hops < max_hops:
        ...
        applicable_only = any(r.get("aggregation") == "APPLICABLE_ONLY" for r in rules)
        verdicts: list[tuple[dict, dict]] = []
        satisfied_applicable = False
        for rule in rules:
            if applicable_only and satisfied_applicable:
                _mark_skipped([rule], "not-applicable: sibling already applicable", step_no)
                continue
            verdict = _evaluate(rule)
        ...
        matched = [(r, v) for (r, v) in verdicts if v["_matched"] and not v["_skipped"]]

        oos_match = next(((r, v) for (r, v) in matched if r.get("is_out_of_scope")), None)
        deny_match = next(((r, v) for (r, v) in matched
                           if r.get("decision_type") in _HALT_DECISION_TYPES
                           and not r.get("is_out_of_scope")), None)
        final_match = any(r.get("is_final") and not r.get("is_out_of_scope")
                          for (r, v) in matched)
        nav_goto = None
        nav_stop = False
        static_goto = None
        for r, v in matched:
            nav = v.get("navigation") if isinstance(v.get("navigation"), dict) else None
            if nav:
        ...
        if oos_match is not None:
            ...
            _skip_rest(ordered_steps, i, visited, steps_by_num,
                       "skipped: prior step out of scope — auditing stopped", _mark_skipped)
            break
        ...
        if target is not None and target > step_no:
            for s in ordered_steps[i + 1:]:
                ...
                if s not in visited:
                    visited.add(s)
                    _mark_skipped(steps_by_num[s],
                                  f"skipped: routed from step {step_no} to step {target}", s)
        ...
        if deny_match is not None:
            terminated_at_shape_id = deny_match[0].get("shape_id", "")
            _skip_rest(ordered_steps, i, visited, steps_by_num,
                       "skipped: claim halted earlier (DENY/STOP)", _mark_skipped)
            break

        if nav_stop or final_match:
            terminate_clean = True
            _skip_rest(ordered_steps, i, visited, steps_by_num,
                       "skipped: audit path ended (terminal/stop)", _mark_skipped)
            break

        i += 1

    if hops >= max_hops:
        logger.warning("execute_shapes claim=%s hit max_hops=%d — routing loop guard tripped",
                       claim_id or "-", max_hops)
    ...
    stages.append({
        ...
        "ms": int((time.time() - t0) * 1000),
        "msg": (f"{evaluated} rules evaluated, {skipped_n} skipped across "
                f"{len(visited)} of {len(ordered_steps)} steps"
                + (" -> TERMINATED_EARLY" if terminated_at_shape_id else "")
                + (" -> stopped (out of scope)" if terminate_clean else "")),
    })
    return _finish(rule_results, stages, terminated_at_shape_id,
                   tool_invocations, tool_results)
...
def _finish(rule_results, stages, terminated_at_shape_id,
            tool_invocations, tool_results):
    out: dict = {
