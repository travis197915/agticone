...
from .agents.a01_intake   import (url_validator, url_normalizer, job_initializer,
                                   redis_job_tracker, mongo_job_logger)
from .agents.a02_fetch    import (next_url_picker, depth_limit_checker, http_fetcher,
                                   local_file_fetcher, content_type_detector,
...
from .agents.a04_parse_docx import (docx_metadata, docx_headings, docx_paragraphs,
                                    docx_tables, docx_code_tables, docx_hyperlinks)
...
from .agents.a08_context    import (eob_code_detector, ex_code_detector, denial_code_detector,
                                    pos_code_detector, revenue_code_detector,
                                    bill_type_detector, modifier_code_detector,
                                    frequency_code_detector, system_action_detector,
                                    cpt_code_detector, entity_list_ref_detector,
                                    code_deduplicator)
from .agents.a09_validate   import (document_completeness, step_sequence, decision_row_check,
                                    code_system_check, link_validator, metadata_validator)
from .agents.a10_write_neo4j import (god_node_writer, pre_section_node_writer,
                                     step_node_writer, rule_node_writer,
                                     annotation_node_writer, code_node_writer,
                                     group_rule_node_writer, sequential_edge_writer,
                                     branch_edge_writer, child_doc_edge_writer,
...
from .agents.a11_write_postgres import (pg_sop_writer, pg_precondition_writer,
                                        ...
                                        pg_annotation_writer, pg_reference_writer,
                                        pg_job_updater, pg_graph_writer)
from .agents.a12_write_mongo import (mongo_raw_writer, mongo_parsed_writer, mongo_job_progress)
from .agents.a13_write_redis import (redis_cache_writer, redis_queue_manager, redis_progress_tracker)
from .agents.a14_links      import (link_classifier, html_link_queue, docx_link_queue,
                                    xlsx_link_queue, pdf_link_queue, unresolved_link_logger,
                                    internal_anchor_mapper, accumulated_doc_appender)
from .agents.a15_control    import (completion_checker, state_clearer, error_handler,
                                    final_summary, job_closer)
from .agents.a16_graph_synthesis import (agent_document_profiler,
    ...
    def node(state: PipelineState) -> dict:
        ...
        if pg_logger:
            row_id = pg_logger.log_stage_start(
                stage_name, doc_url=doc_url,
                doc_format=doc_format, doc_depth=doc_depth,
            )
        ...
        try:
            ...
            if pg_logger:
                pg_logger.log_stage_end(row_id, status="ERROR",
                                        error_detail=str(exc), started_ts=t0)
            raise
        return updates
...
def build_graph(cfg: PipelineConfig) -> StateGraph:
    ...
    # ── Intake ────────────────────────────────────────────────────────────────
    g.add_node("intake_stage", _stage(
        url_validator, url_normalizer, job_initializer,
        redis_job_tracker, mongo_job_logger,
        cfg=cfg, stage_name="intake_stage",
    ))

    # ── BFS pick ──────────────────────────────────────────────────────────────
    g.add_node("pick_next_url", _stage(next_url_picker, cfg=cfg, stage_name="pick_next_url"))

    # ── Fetch ─────────────────────────────────────────────────────────────────
    g.add_node("fetch_stage", _stage(
        depth_limit_checker,
        http_fetcher, local_file_fetcher,
        content_type_detector, extension_detector,
        magic_bytes_detector, content_hasher, duplicate_checker,
        cfg=cfg, stage_name="fetch_stage",
    ))
    ...
    g.add_node("docx_parse", _stage(
        docx_metadata, docx_headings, docx_paragraphs,
        docx_tables, docx_code_tables, docx_hyperlinks,
        cfg=cfg, stage_name="docx_parse",
    ))
    g.add_node("xlsx_parse", _stage(
        xlsx_workbook_type, xlsx_sheet_parser, xlsx_header_detector,
    ...
    # ── Context extraction ────────────────────────────────────────────────────
    g.add_node("context_stage", _stage(
        eob_code_detector, ex_code_detector, denial_code_detector,
        pos_code_detector, revenue_code_detector, bill_type_detector,
        modifier_code_detector, frequency_code_detector,
        system_action_detector, cpt_code_detector,
        entity_list_ref_detector, code_deduplicator,
        cfg=cfg, stage_name="context_stage",
    ))

    # ── Validation ────────────────────────────────────────────────────────────
    g.add_node("validate_stage", _stage(
        document_completeness, step_sequence, decision_row_check,
        code_system_check, link_validator, metadata_validator,
        cfg=cfg, stage_name="validate_stage",
    ))
    ...
    g.add_node("narrative_stage", _stage(
        sop_overview_narrator,
        step_narrative_writer,
        cfg=cfg, stage_name="narrative_stage",
    ))
    ...
    g.add_node("graph_synthesis_stage", _stage(
    ...
    # ── Write — Postgres ──────────────────────────────────────────────────────
    g.add_node("write_postgres", _stage(
        pg_sop_writer,
        ...
        pg_job_updater,
        cfg=cfg, stage_name="write_postgres",
    ))

    # ── Write — MongoDB + Redis ────────────────────────────────────────────────
    g.add_node("write_mongo", _stage(
        mongo_raw_writer, mongo_parsed_writer, mongo_job_progress,
        cfg=cfg, stage_name="write_mongo",
    ))
    g.add_node("write_redis", _stage(
        redis_cache_writer, redis_queue_manager, redis_progress_tracker,
        cfg=cfg, stage_name="write_redis",
    ))

    # ── Link discovery ────────────────────────────────────────────────────────
    g.add_node("link_stage", _stage(
        link_classifier,
        html_link_queue, docx_link_queue, xlsx_link_queue, pdf_link_queue,
        unresolved_link_logger, internal_anchor_mapper,
        accumulated_doc_appender,
        cfg=cfg, stage_name="link_stage",
    ))

    # ── Completion + reset ────────────────────────────────────────────────────
    g.add_node("completion_check", _stage(
        error_handler, completion_checker, state_clearer,
        cfg=cfg, stage_name="completion_check",
    ))

    # ── Final ─────────────────────────────────────────────────────────────────
    g.add_node("final_stage", _stage(
        final_summary, job_closer,
        cfg=cfg, stage_name="final_stage",
    ))
    ...
    g.add_conditional_edges("pick_next_url", _route_after_pick,
    ...
    g.add_conditional_edges("completion_check", _route_completion,
