...
# Run everything for local dev (Django + Celery worker)
make dev
# With Celery Beat (scheduled revision checks — set SOP_REVISION_CHECK_ENABLED=true in .env)
make dev-beat
# Start local Docker infra (Postgres, Redis, Neo4j, Mongo, RabbitMQ)
make infra-up
...
# Optional: Celery Beat scheduler for nightly SOP revision-date checks
# Enable in .env first: SOP_REVISION_CHECK_ENABLED=true
...
# Scheduled revision-date checks (Celery Beat — requires beat process)
SOP_REVISION_CHECK_ENABLED=false
SOP_REVISION_CHECK_HOUR=2
